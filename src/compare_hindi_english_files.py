...
def create_corpora_simple_sentences():
    ...
    hindi_input_file = 'resources/Hindi Parallel Corpus.txt'
    english_input_file = 'resources/English Parallel Corpus.txt'
    eng_output = 'resources/English_simple_corpus.txt'
    efip = open(english_input_file, 'r')
    efop = open(eng_output, 'w')
    lines = efip.readlines()
    line_num = -1
    with open(hindi_input_file) as f:
        for sentence in f:
            if is_simple_sentence(sentence):

                line_num = sentence.split('.')[0]
                efop.write(sentence.strip()+'\n') #get line_num sentence from ef.
